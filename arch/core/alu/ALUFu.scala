...
class AluFU(implicit p: Parameters) extends FunctionalUnit with AluConsts {
  ...
  val src2 = MuxLookup(decoder.decoded.alu_sel2, 0.U(p(XLen).W))(
    Seq(
      ...
      A2_IMM.value.U(SZ_A2.W)    -> imm_utils.genImm(uop_reg.instr, decoder.decoded.imm_type),
  ...
  alu.src1   := src1
  alu.src2   := src2
  alu.fnType := decoder.decoded.alu_fn
  alu.mode   := decoder.decoded.alu_mode
