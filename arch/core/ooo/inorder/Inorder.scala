...
class Inorder(implicit p: Parameters) extends Scheduler {
  ...
  // Create clear masks for when FUs complete their writebacks
  ...
  // Track FU availability combinationally within the cycle
  val temp_fu_avail = Wire(Vec(p(IssueWidth) + 1, Vec(numFUs, Bool())))
  for (i <- 0 until numFUs)
    temp_fu_avail(0)(i) := fu_reqs(i).ready

  // Default FU requests to invalid
  fu_reqs.foreach { req =>
    req.valid := false.B
  ...
  for (w <- 0 until p(IssueWidth)) {
    ...
    // Always accept invalid requests (bubbles) to clear them
    val ready_to_issue = !data_hazard && fu_available && prev_issued
    dis_req.ready := !dis_req.valid || ready_to_issue
  ...
  for (r <- 0 until numRegs) {
    val cleared           = (0 until numFUs).map(i => clear_masks(i)(r)).reduce(_ || _)
