...
class ReorderBuffer(implicit p: Parameters) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val wb_bru_mispredict = Input(Bool())
    val wb_bru_target     = Input(UInt(p(XLen).W))
  ...
  when(io.wb_valid) {
    buffer(io.wb_rob_tag).ready          := true.B
    buffer(io.wb_rob_tag).data           := io.wb_data
    buffer(io.wb_rob_tag).flush_pipeline := io.wb_bru_mispredict || io.wb_trap_req || io.wb_trap_ret
    buffer(io.wb_rob_tag).flush_target   := MuxCase(
      io.wb_bru_target,
      Seq(
