...
object LsuData {
  ...
  def rawMask(ctrl: LsuCtrl)(implicit p: Parameters): UInt = {
    val n = bytes
  ...
  def shiftedMask(ctrl: LsuCtrl, addr: UInt)(implicit p: Parameters): UInt = {
    val n = bytes
    (rawMask(ctrl) << byteOffset(addr))(n - 1, 0)
  }

  def narrowStoreData(ctrl: LsuCtrl, data: UInt)(implicit p: Parameters): UInt =
    MuxCase(
      data,
      Seq(
  ...
  def alignedStoreData(ctrl: LsuCtrl, addr: UInt, data: UInt)(implicit p: Parameters): UInt = {
  ...
  def loadResult(ctrl: LsuCtrl, addr: UInt, alignedData: UInt)(implicit p: Parameters): UInt = {
